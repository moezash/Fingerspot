...
// 3. Prepare Body Data
$data = [
    'trans_id' => uniqid() // A unique identifier for the transaction
];
...
if (curl_errno($ch)) {
    ...
    // Fingerspot API may use 'status' or 'success' to indicate the outcome
    ...
    if ($result && $isSuccess) {
        ...
        if (!empty($result['data'])) {
            foreach ($result['data'] as $device) {
                echo "- SN: " . ($device['cloud_id'] ?? 'N/A') . " | Name: " . ($device['name'] ?? 'Unknown') . " | Status: " . ($device['status'] ?? 'Unknown') . "\n";
            }
        } else {
