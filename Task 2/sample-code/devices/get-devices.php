...
// 3. Prepare Body
// 'trans_id' is used to identify the transaction and is usually a timestamp
$data = [
...
// Fingerspot API expects POST method for most requests
    ...
    if ($result && isset($result['success']) && $result['success']) {
        ...
        if (isset($result['data']) && is_array($result['data'])) {
            foreach ($result['data'] as $device) {
                echo "- Cloud ID: " . $device['cloud_id'] . " | Name: " . ($device['name'] ?? 'N/A') . " | Status: " . ($device['status'] ?? 'Unknown') . "\n";
            }
        } else {
