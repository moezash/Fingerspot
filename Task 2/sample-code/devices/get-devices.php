...
// Fingerspot API expects POST for most requests
curl_setopt($ch, CURLOPT_POST, true);
// Required request body for tracking
    ...
    if ($result && isset($result['success']) && $result['success']) {
        ...
        if (!empty($result['data'])) {
            foreach ($result['data'] as $device) {
                echo "- Cloud ID: " . $device['cloud_id'] . " | Name: " . $device['name'] . "\n";
            }
        } else {
            echo "No devices registered to this account.\n";
        }
    } else {
