 ...
 * - Cloud ID of your Fingerspot machine
 *
...
// 2. Prepare Data
// Recommendation: Keep the date range small (max 2 days) for better performance.
$data = [
    ...
    if ($result && $isSuccess) {
        ...
        if (isset($result['data']) && is_array($result['data']) && !empty($result['data'])) {
            foreach ($result['data'] as $log) {
                $pin  = $log['pin'] ?? 'N/A';
                $scan = $log['scan'] ?? 'Unknown Time';
                $status = $log['status_scan'] ?? '0'; // 0 = Check-in, 1 = Check-out, etc.
                echo "- PIN: $pin | Time: $scan | Status: $status\n";
            }
        } else {
