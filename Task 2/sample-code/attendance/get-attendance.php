 ...
 * Requirements:
 * - Pure PHP + cURL
 * - Valid API Token and Device Cloud ID
 *
...
// 2. Prepare Data
// Fingerspot recommends a max range of 2 days for stability.
$data = [
    ...
    if ($result && (isset($result['status']) && $result['status'] || isset($result['success']) && $result['success'])) {
        ...
        if (isset($result['data']) && !empty($result['data'])) {
            foreach ($result['data'] as $log) {
                echo "- PIN: " . $log['pin'] . "\n";
                echo "  Time: " . $log['scan'] . "\n";
                echo "  Status: " . ($log['status_scan'] ?? '0') . " (0: In, 1: Out)\n";
                echo "  Verify: " . ($log['verify'] ?? '1') . " (1: Finger, 2: Pass, 3: Card)\n";
                echo "--------------------------\n";
            }
        } else {
...
---------------------------
{
    ...
    "data": [
        {
            "pin": "1",
            "scan": "2024-06-01 08:00:15",
            "verify": "1",
            "status_scan": "0"
        },
        {
            "pin": "1",
            "scan": "2024-06-01 17:05:30",
            "verify": "1",
            "status_scan": "1"
        }
    ]
