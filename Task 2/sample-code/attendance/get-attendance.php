...
// Note: Fingerspot API typically allows a max range of 2 days for get_attlog.
$data = [
...
if (curl_errno($ch)) {
    ...
    // Check for success using both potential keys
    if ($result && ((isset($result['status']) && $result['status']) || (isset($result['success']) && $result['success']))) {
        ...
        if (isset($result['data']) && is_array($result['data']) && !empty($result['data'])) {
            ...
            foreach ($result['data'] as $log) {
                echo str_pad($log['pin'] ?? 'N/A', 10) . " | " .
                     str_pad($log['scan'] ?? 'N/A', 20) . " | " .
                     ($log['status_scan'] ?? 'N/A') . "\n";
            }
        } else {
