...
// 1. Configuration
// Get your API Token and Cloud ID from the Fingerspot Developer Dashboard
...
// 2. Prepare Data
/**
 * NOTE: For optimal performance and reliability, Fingerspot recommends
 * a maximum range of 2 days per request for attendance logs.
 */
$data = [
    ...
    if ($result && $isSuccess) {
        ...
        if (isset($result['data']) && !empty($result['data'])) {
            foreach ($result['data'] as $log) {
                ...
                // Status_scan: 0 for In, 1 for Out (depending on device settings)
                echo "- PIN: " . $log['pin'] . " | Time: " . $log['scan'] . " | Status: " . $log['status_scan'] . " | Verify: " . $log['verify'] . "\n";
            }
        } else {
