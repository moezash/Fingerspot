<?php
/**
 * Sample code for Setting User Information on Fingerspot Device
 *
 * This sample demonstrates how to upload employee data (PIN and Name)
 * to the attendance machine remotely.
 *
 * Documentation: https://developer.fingerspot.io
 */

// 1. Configuration
$apiToken = 'YOUR_API_TOKEN_HERE';
$cloudId  = 'YOUR_CLOUD_ID_HERE';
$apiUrl   = 'https://developer.fingerspot.io/api/set_userinfo';

// 2. Prepare User Data
$data = [
    'trans_id'  => (string)time(),
    'cloud_id'  => $cloudId,
    'pin'       => '101',          // Employee PIN / ID
    'name'      => 'John Doe',     // Employee Name
    'privilege' => '0',            // 0: User, 1: Admin
    'password'  => '',             // Optional device password
    'rfid'      => ''              // Optional RFID Card ID
];

// 3. Prepare Headers
$headers = [
    'Authorization: Bearer ' . $apiToken,
    'Content-Type: application/json',
    'Accept: application/json'
];

// 4. Initialize cURL
$ch = curl_init($apiUrl);

// 5. Set cURL Options
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
curl_setopt($ch, CURLOPT_POST, true);
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($data));
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, true);

// 6. Execute Request
$response = curl_exec($ch);

// 7. Check for errors
if (curl_errno($ch)) {
    echo "cURL Error: " . curl_error($ch) . "\n";
} else {
    $result = json_decode($response, true);
    echo "--- Set User Information Sample ---\n";
    echo "Sending PIN: " . $data['pin'] . " (" . $data['name'] . ")\n";

    if ($result && isset($result['status']) && $result['status']) {
        echo "Command sent successfully. The machine will process this and report via Webhook.\n";
    } else {
        echo "Failed to send command.\n";
        echo "Response: " . $response . "\n";
    }
}

curl_close($ch);

/*
Example Request Body:
--------------------
{
    "trans_id": "1705824000",
    "cloud_id": "FTV123456",
    "pin": "101",
    "name": "John Doe",
    "privilege": "0"
}

Example Response (Success):
--------------------
{
    "status": true,
    "message": "Success"
}
*/
?>
