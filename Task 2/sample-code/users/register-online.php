 ...
 * a registration process for a specific user remotely.
 *
 * Requirements:
 * - PHP cURL extension
...
$data = [
    ...
    'verification' => '0'       // Type of registration:
                                // 0-9: Fingerprint index
                                // 12: Face
                                // 13: Palm Vein
];
...
if (curl_errno($ch)) {
    ...
    echo "Triggering registration for PIN " . $data['pin'] . " (Mode: " . $data['verification'] . ") on device " . $cloudId . "\n";
    echo "HTTP Status Code: $httpCode\n\n";

    if ($result && isset($result['status']) && $result['status']) {
        echo "Command sent successfully. The machine will now enter registration mode.\n";
        echo "Note: The final registration result will be reported to your Webhook.\n";
    } else {
...
---------------------------------------------------------------------------
Example Webhook Payload (Result of registration):
---------------------------------------------------------------------------
{
    "type": "reg_online",
    "cloud_id": "FTV123456",
